...
describe('CircularLinkedList', () => {
  ...
  it('removes valid elements', () => {
    ...
    for (let i: number = min; i <= max; i++) {
      ...
      expect(element).toEqual(i);
      currentMin++;
      verifyList(list, currentMin, max);
    }
  });
  ...
  it('removes element from middle of list', () => {
    ...
    expect(node!.next!.element).toEqual(min);
  });
  ...
  it('returns toString primitive types: string', () => {
    const list: CircularLinkedList<string> = new CircularLinkedList<string>(defaultEquals);
  ...
  it('returns toString objects', () => {
    const list: CircularLinkedList<MyObject> = new CircularLinkedList<MyObject>();
